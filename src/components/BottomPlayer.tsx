...
export const BottomPlayer: React.FC = () => {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>, originalUrl?: string, title?: string) => {
    ...
    if (!target.dataset.proxied && originalUrl && !originalUrl.startsWith("/api/proxy-image")) {
      ...
      target.src = `/api/proxy-image?url=${encodeURIComponent(originalUrl)}&title=${encodeURIComponent(title || currentEpisode?.title || "")}`;
    } else {
      target.src = `/api/proxy-image?title=${encodeURIComponent(title || currentEpisode?.title || "Podcast")}`;
    }
  };
